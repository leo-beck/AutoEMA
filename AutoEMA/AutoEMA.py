...
class BaseModel:
    ...
    def plot_stability_diagram(self, show_poles: bool = True, show_nf: bool = True, x_lim: tuple = None,
        ...
        fig.suptitle("Stability diagram", fontsize=font_size)
        ...
        # Add legend
        fig.legend(bbox_to_anchor=(1, 0.88), loc="upper left", borderaxespad=0,
                   prop={'size': font_size}, fancybox=True, framealpha=1, labelspacing=label_spacing)
    ...
    def plot_damping_ratios(self, path_to_save: str = None):
        ...
        plt.legend(bbox_to_anchor=(1.04, 1), loc="upper left", borderaxespad=0,
                   prop={'size': font_size}, fancybox=True, framealpha=1, labelspacing=0.3)
        # Export if path is defined
        if path_to_save is not None:
            plt.savefig(path_to_save, bbox_inches='tight')
        plt.show()
...
class OptModel(BaseModel):
    def __init__(self, frf: np.ndarray, f_axis: np.ndarray, lowest_f: int = None, highest_f: int = None,
        ...
        :param order: The model order to optimize for. If None, the model order is calculated automatically.
        :param show_progress: Prints the result of every step of the bayesian optimization.
        :param reg: Regularization factor. Increasing values leed to smaller model orders.
